...
static int mov_read_infe(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (version == 0) {
        // Version 0: item_ID(16), item_protection_index(16), item_name(string),
        //            content_type(string), content_encoding(string, optional)
        if (size < 4)
            return AVERROR_INVALIDDATA;
        ...
        // content_type (null-terminated string, skip for now)
        if (size > 0) {
            AVBPrint bprint;
            av_bprint_init(&bprint, 0, AV_BPRINT_SIZE_UNLIMITED);
            ret = ff_read_string_to_bprint_overwrite(pb, &bprint, size);
            if (ret >= 0)
                size -= ret + 1;
            av_bprint_finalize(&bprint, NULL);
        }

        // content_encoding (optional, skip)
        if (size > 0)
            avio_skip(pb, size);

        item_type = 0; // version 0 uses content_type, not item_type
        ret = 1;       // ensure item_name is saved in av_bprint_finalize below

        av_log(c->fc, AV_LOG_TRACE, "infe: item_id %d, item_name %s (version 0)\n",
               item_id, item_name.str);
    } else if (version < 2) {
        // version 1: not yet supported
        avpriv_report_missing_feature(c->fc, "infe version 1");
        avio_skip(pb, size);
        return 1;
    } else {
